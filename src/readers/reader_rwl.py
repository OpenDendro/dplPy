...
# Description: Reading in .rwl type file. Assigning variables to each value in the Tucson format data sets. 
# example usage:
...
# define Read RWL module
def read_rwl(input):
    try: 
        if input.endswith('.rwl'):
            with open('rwldir' +'/'+ input, 'r') as rings:
                data= rings.read()
                lines = data.split("\n")
                print(lines[0-2])
            #read every line
    ...
    # Echo error if file is corrupt            
        ...
        print('Bad file:', input) # print out the name of the corrupt file
