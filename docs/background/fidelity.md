...
| **`powt` power vs ARSTAN** | The adaptive power is fit by regressing log(spread) on log(level) across adjacent rings. dplPy (following dplR) uses the pair **mean** `(x[t]+x[t-1])/2` as the level and applies `p = |1-b|` unclamped; ARSTAN's option 4 uses the **trailing ring** `x[t]` as the level and clamps `p` to `[0, 1]`. | dplPy reproduces dplR's `powt` exactly (its primary reference); a series' power can therefore differ slightly from ARSTAN's. |
